...
from PyQt5.QtWidgets import (QWidget, QVBoxLayout, QLabel, QListWidget, QListView,
...
class BoxWidget(QWidget):
    ...
    def __init__(self, box_id: str, name: str, config: dict, global_border_settings: dict,
        ...
        self.file_list.item_dragged_out.connect(self._remove_dragged_item)
    ...
    def _on_item_double_clicked(self, item: QListWidgetItem):
        ...
        if file_path:
            try:
                self.logger.info(f"Попытка запустить файл: {file_path}")
                os.startfile(file_path)
            except Exception as e:
                self.logger.error(f"Не удалось запустить файл '{file_path}': {e}")
    ...
    def apply_styles(self):
        appearance = self.config.get("appearance", {})
        bg_color_hex = appearance.get("color", "#2D2D2D")
        # --- НАЧАЛО ИЗМЕНЕНИЯ: Читаем все глобальные настройки ---
    ...
    def _get_file_icon(self, path_str: str) -> QIcon:
        ...
        try:
            flags = win32con.SHGFI_ICON | win32con.SHGFI_USEFILEATTRIBUTES | 0x000000000
            h_icon, _, _ = win32gui.SHGetFileInfo(path_str, 0, flags)
            if h_icon == 0:
                return QIcon.fromTheme("document-new")

            pixmap = QPixmap.fromImage(QImage.fromHICON(h_icon))
            win32gui.DestroyIcon(h_icon)
            return QIcon(pixmap)
        except Exception as e:
            self.logger.warning(f"Не удалось извлечь иконку для {path_str}: {e}")
            return QIcon.fromTheme("document-new")
